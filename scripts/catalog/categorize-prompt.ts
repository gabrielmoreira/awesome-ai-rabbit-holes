...
// Catalog prompt and response boundary. Generates insights, tags, and a persisted
...
Catalog scope:
- practical AI tools, agents, workflows, infrastructure, and app-building products relevant to software developers
- coding agents, app builders, agent orchestration, MCP tooling, AI IDEs, local AI, evals, developer workflow automation, and adjacent developer-facing infrastructure
...
- Use the available category list below as the source of truth.
- Each category line includes its id, name, description, and a prompt-specific fit instruction.
- Choose exactly one category only when the project clearly fits that category's instruction.
- Browser-first prompt-to-app or prompt-to-site SaaS products belong under app-builders when that category is available.
- Do not put a hosted app builder into coding-agents just because the marketing mentions agents, code generation, or autonomous workflows.
- Reserve coding-agents for tools that collaborate with the developer on source code inside a repo, IDE, CLI, PR, or code review workflow.
- If nothing fits cleanly, exclude the project instead of forcing a bad category.
...
Available categories (id | name | description | prompt instruction):
${categorySection}
...
- if the product mainly generates whole apps, sites, or internal tools from prompts inside a hosted builder surface, prefer app-builders over coding-agents.
...
- why_it_matters: honest, useful. Grounded in what the README or source-list context says the tool changes for developers. For app builders, talk about what they help a developer ship or prototype, not about "manual coding" disappearing unless the source explicitly frames it that way.

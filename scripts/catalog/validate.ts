...
export function validateOverride(override: Override, items: CatalogItem[]): ValidationError[] {
  ...
  if (!override.id) errors.push({ path: "override", message: "Override missing required field: id" });
  ...
  if (!targetItem) {
    errors.push({ path: override.id ?? "unknown", message: `Override targets unknown item id: ${override.id}` });
  }

  const rawPatch = override.patch as unknown;
  if (rawPatch === null || rawPatch === undefined || typeof rawPatch !== "object" || Array.isArray(rawPatch)) {
    errors.push({ path: override.id ?? "unknown", message: "Override patch must be a plain object" });
    return errors;
  }

  const allowed = new Set(["insights", "curation", "placement", "lifecycle"]);
  const patchObject = rawPatch as Record<string, unknown>;
  for (const field of Object.keys(patchObject)) {
  ...
  for (const field of allowed) {
    if (!(field in patchObject)) continue;
    const subPatch = patchObject[field];
    if (subPatch === null || subPatch === undefined || typeof subPatch !== "object" || Array.isArray(subPatch)) {
      errors.push({ path: override.id ?? "unknown", message: `Override patch.${field} must be a plain object` });
    }
  }
...
export function validateOverridesUniqueness(overrides: Override[]): ValidationError[] {
  ...
  for (const override of overrides) {
    ...
    seen.set(override.id, (seen.get(override.id) ?? 0) + 1);
  }
  for (const [id, count] of seen) {
    if (count > 1) errors.push({ path: id, message: `Duplicate override id: ${id} (found ${count} times)` });
  }
  return errors;
}

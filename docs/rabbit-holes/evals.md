...
- **[langfuse](https://github.com/langfuse/langfuse)** `⭐ 26.6k` `updated ≤30d` An open source LLM engineering platform providing observability, metrics, evaluations, prompt management, and a playground for debugging and improving AI applications. <details><summary>More about</summary>
  ...
  `llm-observability` `evals` `prompt-management` `self-hosted` `llmops`
  </details>
...
- **[opik](https://github.com/comet-ml/opik)** `⭐ 19.2k` `updated ≤30d` Opik is an open-source platform for tracing, evaluating, and monitoring LLM applications, RAG systems, and agentic workflows with production dashboards. <details><summary>More about</summary>
  ...
  `llm-observability` `evaluation` `agent-monitoring` `tracing` `llmops`
  </details>
...
- **[deepeval](https://github.com/confident-ai/deepeval)** `⭐ 15.2k` `updated ≤30d` An open-source Python framework for unit testing LLM applications using research-backed metrics like G-Eval, hallucination detection, and task completion. <details><summary>More about</summary>
  ...
  `evals` `llm-evaluation` `testing` `python` `metrics`
  </details>

- **[ragas](https://github.com/explodinggradients/ragas)** `⭐ 13.8k` `updated ≤90d` Ragas is a Python toolkit for evaluating and optimizing LLM applications through objective metrics, automated test data generation, and production-data feedback loops. <details><summary>More about</summary>
  ...
  `llm-evals` `rag` `testing` `metrics` `python`
  </details>
...
- **[phoenix](https://github.com/arize-ai/phoenix)** `⭐ 9.5k` `updated ≤30d` An open-source platform for AI observability, tracing, and evaluation, designed to help developers instrument and optimize LLM applications and agents. <details><summary>More about</summary>
  ...
  `llm-eval` `observability` `tracing` `opensource`
  </details>
...
- **[evidently](https://github.com/evidentlyai/evidently)** `⭐ 7.4k` `updated ≤30d` Evidently is an open-source Python framework for evaluating, testing, and monitoring ML and LLM-powered systems across both experimentation and production environments. <details><summary>More about</summary>
  ...
  `mlops` `llm-observability` `evals` `monitoring` `python`
  </details>
...
- **[opencompass](https://github.com/open-compass/opencompass)** `⭐ 7k` `updated ≤30d` OpenCompass is an LLM evaluation platform that benchmarks a wide range of models against over 100 datasets using a Python-based evaluation harness. <details><summary>More about</summary>
  ...
  `llm` `benchmark` `evaluation` `testing`
  </details>
...
- **[greptimedb](https://github.com/greptimeteam/greptimedb)** `⭐ 6.2k` `updated ≤30d` GreptimeDB is an open-source Rust-based observability database that unifies metrics, logs, and traces into a single engine with SQL and PromQL support, designed to replace Prometheus, Loki, and Elasticsearch. <details><summary>More about</summary>
  ...
  `observability` `database` `llm-monitoring` `opentelemetry` `rust`
  </details>
...
- **[helicone](https://github.com/helicone/helicone)** `⭐ 5.6k` `updated ≤30d` Helicone is an open-source AI gateway and LLM observability platform that proxies model requests to provide logging, cost tracking, prompt management, and agent tracing with a single base URL change. <details><summary>More about</summary>
  ...
  `llm-observability` `ai-gateway` `monitoring` `prompt-management` `self-hosted`
  </details>

- **[giskard](https://github.com/giskard-ai/giskard)** `⭐ 5.3k` `updated ≤30d` Giskard is an open-source Python library for testing, evaluating, and red-teaming LLM agents, featuring scenario-based evals, built-in checks, and multi-turn conversation testing. <details><summary>More about</summary>
  ...
  `llm-eval` `red-teaming` `agent-testing` `python-library` `rag-evaluation`
  </details>

- **[giskard](https://github.com/giskard-ai/giskard-oss)** `⭐ 5.3k` `updated ≤30d` Giskard is an open-source Python library for evaluating, testing, and red-teaming LLM agents and RAG systems, featuring scenario-based evals and built-in vulnerability scanning. <details><summary>More about</summary>

  It gives developers a structured way to catch regressions and security issues in non-deterministic agentic workflows before they reach production.

  _Because nothing says 'trust the autonomous agent' like needing a dedicated framework just to confirm it won't leak your database credentials via prompt injection._

  `llm-eval` `red-teaming` `agent-testing` `rag-evaluation` `python-library`
  </details>
...
- **[autorag](https://github.com/marker-inc-korea/autorag)** `⭐ 4.7k` `updated ≤30d` AutoRAG is an open-source Python framework that automates the evaluation and optimization of Retrieval-Augmented Generation (RAG) pipelines using AutoML-style experimentation on your own data. <details><summary>More about</summary>
  ...
  `rag` `automl` `evaluation` `llm-ops` `python`
  </details>
...
- **[vlmevalkit](https://github.com/open-compass/vlmevalkit)** `⭐ 4.1k` `updated ≤30d` An open-source evaluation toolkit for large vision-language models that supports one-command benchmarking across 220+ LMMs and 80+ benchmarks. <details><summary>More about</summary>
  ...
  `eval` `vision-language` `benchmark` `multimodal` `llm`
  </details>
...
- **[agenta](https://github.com/agenta-ai/agenta)** `⭐ 4.1k` `updated ≤30d` Agenta is an open-source LLMOps platform providing a prompt playground, prompt management, LLM evaluation, and observability for engineering and product teams building LLM applications. <details><summary>More about</summary>
  ...
  `llmops` `prompt-management` `evals` `observability` `open-source`
  </details>
...
- **[langwatch](https://github.com/langwatch/langwatch)** `⭐ 3.2k` `updated ≤30d` LangWatch is an open-source platform for end-to-end LLM evaluation, AI agent simulation, and production observability, supporting OpenTelemetry-native tracing and an AI gateway for cost control. <details><summary>More about</summary>
  ...
  `llm-evals` `observability` `agent-testing` `llmops` `opentelemetry`
  </details>
...
- **[lighteval](https://github.com/huggingface/lighteval)** `⭐ 2.4k` `updated ≤30d` Lighteval is a Python toolkit from Hugging Face for evaluating LLMs across more than 1,000 tasks and multiple inference backends, with support for custom metrics and sample-level result inspection. <details><summary>More about</summary>
  ...
  `evaluation` `llm` `benchmark` `huggingface` `metrics`
  </details>
...
- **[evaluation-guidebook](https://github.com/huggingface/evaluation-guidebook)** `⭐ 2.1k` `updated ≤180d` A guidebook from Hugging Face sharing practical insights and theoretical knowledge on evaluating LLM performance, covering automated benchmarks, human evaluation, and LLM-as-a-judge methods. <details><summary>More about</summary>
  ...
  `llm-evaluation` `benchmarks` `huggingface` `guide`
  </details>
...
- **[uqlm](https://github.com/cvs-health/uqlm)** `⭐ 1.1k` `updated ≤30d` UQLM is a Python library that provides uncertainty quantification techniques and confidence scoring to detect hallucinations in LLM outputs across black-box, white-box, LLM-as-a-judge, ensemble, and long-text scorers. <details><summary>More about</summary>
  ...
  `hallucination-detection` `llm-evaluation` `uncertainty-quantification` `langchain`
  </details>
...
- **[skills](https://github.com/nvidia-nemo/skills)** `⭐ 946` `updated ≤30d` Nemo Skills is a pipeline collection for training, evaluating, and running inference on LLMs, supporting benchmarks from math and code to tool-calling and long-context tasks. <details><summary>More about</summary>
  ...
  `llm-training` `evals` `benchmarks` `nvidia` `inference`
  </details>

- **[dingo](https://github.com/dataeval/dingo)** `⭐ 693` `updated ≤30d` Dingo is a Python toolkit and SaaS platform for evaluating the quality of AI training data, models, and RAG applications using a mix of rule-based checks, LLM-as-a-judge, and agent-based assessments. <details><summary>More about</summary>
  ...
  `evals` `data-quality` `llm-judge` `rag` `mcp`
  </details>

- **[dingo](https://github.com/migoxlab/dingo)** `⭐ 693` `updated ≤30d` Dingo is a Python-based evaluation tool for assessing the quality of AI training data, models, and RAG applications using a mix of heuristic rules, LLM-based checks, and agent-based evaluations. <details><summary>More about</summary>

  It provides developers with a scalable pipeline to catch hallucinations, validate datasets, and benchmark RAG performance before models hit production.

  _Yet another reminder that we now need dedicated agents to verify the output of the agents we just hired to write the code._

  `evals` `data-quality` `rag` `llm-judge`
  </details>
...
- **[evalchemy](https://github.com/mlfoundations/evalchemy)** `⭐ 594` `updated ≤90d` A unified toolkit for evaluating post-trained language models across multiple benchmarks with support for local and API-based models. <details><summary>More about</summary>
  ...
  `evals` `llm` `benchmarks` `reasoning`
  </details>
...
- **[openjudge](https://github.com/agentscope-ai/openjudge)** `⭐ 587` `updated ≤30d` OpenJudge is an open-source evaluation framework for AI applications that provides ready-to-use and scenario-specific graders to assess quality and generate reward signals for fine-tuning. <details><summary>More about</summary>
  ...
  `evaluation` `grading` `ai-agents` `benchmark` `fine-tuning`
  </details>

- **[openjudge](https://github.com/modelscope/openjudge)** `⭐ 587` `updated ≤30d` OpenJudge is an open-source framework and hosted playground for evaluating the quality of AI applications, agents, and chatbots using ready-to-use and custom rubric graders. <details><summary>More about</summary>

  It provides developers with a structured workflow to measure agent performance, generate reward signals for fine-tuning, and iterate on application quality beyond simple output checks.

  _We have now reached the point where we need a dedicated evaluation framework just to figure out if our evaluation framework's graders are actually grading the agents that are grading the code._

  `evals` `agents` `benchmark` `open-source`
  </details>
...
- **[yourbench](https://github.com/huggingface/yourbench)** `⭐ 444` `updated ≤90d` A Hugging Face framework that transforms source documents like PDFs and Word files into structured LLM evaluation datasets with configurable QA schemas. <details><summary>More about</summary>
  ...
  `llm-eval` `benchmarking` `huggingface` `datasets` `cli`
  </details>
...
- **[ollama-benchmark](https://github.com/aidatatools/ollama-benchmark)** `⭐ 359` `updated ≤180d` A cross-platform CLI tool that benchmarks local LLMs running via Ollama by measuring throughput and tokens-per-second performance. <details><summary>More about</summary>
  ...
  `benchmark` `ollama` `local-llm` `cli` `evaluation`
  </details>
...
- **[mixeval](https://github.com/jinjieni/mixeval)** `⭐ 256` `updated >1y` MixEval is a dynamic LLM evaluation suite and benchmark framework that correlates highly with Chatbot Arena rankings while running locally with significantly lower cost and time. <details><summary>More about</summary>
  ...
  `llm-eval` `benchmark` `model-evaluation` `arena-alternative`
  </details>
...
- **[medevalkit](https://github.com/alibaba-damo-academy/medevalkit)** `⭐ 228` `updated ≤90d` MedEvalKit is a unified evaluation framework for benchmarking large medical language and multimodal models across a wide range of healthcare datasets and supported model architectures. <details><summary>More about</summary>
  ...
  `evaluation` `medical-ai` `llm` `benchmark` `multimodal`
  </details>
...
- **[hpolib2](https://github.com/automl/hpobench)** `⭐ 167` `updated ≤1y` HPOBench is a library providing containerized hyperparameter optimization benchmarks designed for reproducible multi-fidelity benchmarking. <details><summary>More about</summary>
  ...
  `hyperparameter-optimization` `benchmarks` `reproducibility` `ml-eval`
  </details>

- **[hpolib2](https://github.com/automl/hpolib2)** `⭐ 167` `updated ≤1y` A library of containerized hyperparameter optimization benchmarks designed for reproducible (multi-fidelity) HPO evaluation. <details><summary>More about</summary>

  It provides developers and researchers with standardized, isolated benchmarks to rigorously measure and compare optimization algorithms without dependency conflicts.

  _Because nothing says 'cutting-edge AI research' like spending three days debugging a Singularity container just to prove your random forest tuned itself slightly better._

  `hyperparameter-optimization` `benchmarks` `automl` `reproducibility` `evaluation`
  </details>

- **[plexiglass](https://github.com/kortex-labs/plexiglass)** `⭐ 154` `updated ≤90d` A CLI toolkit for testing LLMs against adversarial attacks like prompt injection and jailbreaking, while benchmarking models for security, bias, and toxicity. <details><summary>More about</summary>
  ...
  `llm-security` `cli` `benchmarking` `evals` `adversarial-attacks`
  </details>
...
- **[ragtune](https://github.com/metawake/ragtune)** `⭐ 11` `updated ≤90d` A CLI tool for debugging, benchmarking, and monitoring the retrieval layer of RAG pipelines with support for multiple vector stores and CI/CD integration. <details><summary>More about</summary>
  ...
  `rag` `evals` `cli` `vector-search` `debugging`
  </details>
...
- **[slopcodemonitor.ai](https://github.com/marcoramilli/slopcodemonitor.ai)** `⭐ 0` `updated ≤30d` A daily automated scanner that analyzes trending GitHub repositories to detect and score AI-generated code patterns using the SynthScan fingerprint engine. <details><summary>More about</summary>
  ...
  `code-analysis` `code-quality` `ai-detection` `github-scanner`
  </details>
...
- **[synthscan](https://github.com/marcoramilli/synthscan)** `⭐ 0` `updated ≤30d` SynthScan is a GitHub Action and local scanner that detects AI-generated code patterns in a repository using a configurable set of rules and reports a normalized 'Synthetic Code Score'. <details><summary>More about</summary>
  ...
  `code-review` `ai-detection` `github-actions` `code-quality`
  </details>

- **[superagent](https://github.com/superagent-ai/superagent)** A red teaming service that deploys specialized attack agents against AI systems in production to surface data leaks, harmful outputs, and unauthorized actions. <details><summary>More about</summary>

  Developers building AI agents can use this to find security and compliance gaps before they result in real-world failures or customer data exposure.

  _We have finally reached the point where we need autonomous agents to attack our autonomous agents because we no longer have the bandwidth to break our own stuff._

  `agents` `ai-safety` `red-teaming` `security`
  </details>

- **[arena.ai](https://arena.ai)** A browser-based platform for chatting with and comparing multiple top LLMs side-by-side using crowdsourced benchmarks and leaderboards. <details><summary>More about</summary>
  ...
  `llm-comparison` `benchmarking` `multi-model` `evals`
  </details>

- **[designarena.ai](https://designarena.ai)** Design Arena is a crowdsourced benchmark platform that pits top AI models against the same creative design prompts and lets users vote on the best outputs to power public leaderboards. <details><summary>More about</summary>
  ...
  `benchmark` `eval` `design` `model-comparison`
  </details>

- **[evaluations](https://weave-docs.wandb.ai/guides/core-types/evaluations)** Weave Evaluations is a feature within Weights & Biases' Weave toolkit for defining, running, and tracking evaluations of LLM outputs and application behavior. <details><summary>More about</summary>

  It provides developers with structured tooling to measure model quality and catch prompt regressions before they reach production.

  _Finally, a rigorous way to prove that the new prompt isn't actually better, just differently wrong._

  `evals` `llm-observability` `testing` `wandb`
  </details>

- **[fiddler-labs](https://github.com/fiddler-labs)** Fiddler AI is a company GitHub organization hosting repos for AI observability, including fiddler-auditor for evaluating language models and tooling for ML monitoring. <details><summary>More about</summary>
  ...
  `llm-eval` `ai-observability` `ml-monitoring`
  </details>

- **[home](https://agi-eval.cn/mvp/home)** AGI-Eval is an evaluation benchmark and platform for assessing the capabilities of AI models on tasks related to artificial general intelligence. <details><summary>More about</summary>
  ...
  `evals` `benchmark` `agi` `llm-evaluation`
  </details>

- **[literalai.com](https://literalai.com)** LiteralAI is a platform for observing, evaluating, and managing LLM applications, included in the Awesome-LLMOps ecosystem. <details><summary>More about</summary>
  ...
  `llmops` `observability` `evaluation` `platform`
  </details>

- **[llm-leaderboard.streamlit.app](https://llm-leaderboard.streamlit.app)** A Streamlit web application that provides a leaderboard for comparing the performance of various large language models. <details><summary>More about</summary>
  ...
  `llm` `benchmark` `leaderboard` `eval`
  </details>

- **[llmops](https://www.fiddler.ai/llmops)** Fiddler AI is an enterprise LLMOps platform that provides observability, guardrails, and governance tools for monitoring and safeguarding production LLM and agentic applications. <details><summary>More about</summary>
  ...
  `llmops` `observability` `guardrails` `enterprise` `agentic`
  </details>

- **[open_llm_leaderboard](https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard)** A Hugging Face Space that tracks, ranks, and evaluates open large language models and chatbots. <details><summary>More about</summary>
  ...
  `llm` `benchmark` `evaluation` `huggingface`
  </details>

- **[promptlayer.com](https://promptlayer.com)** A platform for versioning, testing, and monitoring prompts and AI agents, featuring evaluation tools, tracing, and a visual editor for collaboration. <details><summary>More about</summary>
  ...
  `llmops` `prompt-management` `evals` `monitoring` `agents`
  </details>

- **[respan.ai](https://respan.ai)** Respan is a hosted LLM engineering platform that unifies observability, evaluations, prompt optimization, and a gateway for AI agents and applications. <details><summary>More about</summary>
  ...
  `llm-ops` `observability` `evals` `prompt-optimization` `gateway`
  </details>

- **[scorable.ai](https://scorable.ai)** Scorable is a SaaS platform for creating custom AI judges and running evaluations to monitor and improve the quality of LLM application outputs in production. <details><summary>More about</summary>
  ...
  `llm-evals` `monitoring` `ai-quality` `guardrails`
  </details>

- **[vibedoctor.io](https://vibedoctor.io)** A security and code quality scanner that runs checks across 15 areas and 129+ rules against AI-generated codebases, offering fix prompts and MCP integration for IDEs. <details><summary>More about</summary>
  ...
  `security` `code-quality` `ai-generated-code` `mcp` `scanner`
  </details>

- **[www.chatarena.org](https://www.chatarena.org)** ChatArena is a platform for evaluating and letting language models compete or collaborate in multi-agent conversational scenarios. <details><summary>More about</summary>
  ...
  `multi-agent` `evaluation` `llm` `research`
  </details>

- **[www.getmaxim.ai](https://www.getmaxim.ai)** Maxim AI is an end-to-end evaluation and observability platform for AI agents, providing infrastructure for prompt experimentation, agent simulation, and production monitoring. <details><summary>More about</summary>
  ...
  `evals` `observability` `agents` `llm-gateway`
  </details>

- **[www.parea.ai](https://www.parea.ai)** Parea AI is a platform for experiment tracking, human annotation, and observability designed for teams building and deploying LLM applications. <details><summary>More about</summary>
  ...
  `llmops` `evals` `observability` `experiment-tracking`
  </details>

- **[www.superagent.sh](https://www.superagent.sh)** A Y Combinator-backed security platform that deploys specialized attack agents to red team production AI systems, uncovering data leaks and harmful outputs before users encounter them. <details><summary>More about</summary>

  Developers building agentic workflows can integrate automated red teaming to catch compliance violations and unauthorized actions that standard system prompts fail to prevent.

  _We now need autonomous agents to attack our autonomous agents, because apparently the only thing more terrifying than one rogue AI is an entire simulated army of them finding your edge cases first._

  `red-teaming` `security` `agent-evals` `production-testing`
  </details>

- **[www.swebench.com](https://www.swebench.com)** A leaderboard website tracking and comparing the performance of AI coding agents on the SWE-bench benchmark of real-world software engineering tasks. <details><summary>More about</summary>
  ...
  `benchmarks` `evals` `coding-agents` `leaderboard`
  </details>
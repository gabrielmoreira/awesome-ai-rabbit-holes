...
import { buildMiseTaskArgs, buildMiseTaskEnv, PI_FREE_TASK_NAME, stripMiseTaskPrelude } from "../scripts/support/runner.js";
import { resolveCatalogLlmModel, resolveCatalogLlmTimeoutMs } from "../scripts/catalog/llm-gateway.js";

describe("buildMiseTaskArgs", () => {
  it("spawns the shared pi:free mise task in stdin mode", () => {
    expect(buildMiseTaskArgs(PI_FREE_TASK_NAME, ["--stdin"])).toEqual([
      "run",
      "pi:free",
      "--",
      "--stdin",
  ...
  it("keeps the prompt off the command line", () => {
    expect(buildMiseTaskArgs(PI_FREE_TASK_NAME, ["--stdin"]).join(" ")).not.toContain("Reply with OK.");
  });

  it("appends an explicit model override when requested", () => {
    expect(buildMiseTaskArgs(PI_FREE_TASK_NAME, ["--stdin", "--model", "openrouter/openai/gpt-oss-120b:free"])).toEqual([
      "run",
      "pi:free",
      "--",
      "--stdin",
...
describe("buildMiseTaskEnv", () => {
  it("keeps provider credentials while dropping npm lifecycle variables", () => {
    ...
    const taskEnv = buildMiseTaskEnv({

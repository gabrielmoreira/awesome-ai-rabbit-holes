...
describe("processing-state helpers", () => {
  ...
  it("claimed-work heartbeat returns a structured snapshot", () => {
    ...
    expect(hb.phase).toBe("categorize");
    expect(hb.done).toBe(4);
    expect(hb.ok).toBe(2);
    expect(hb.fail).toBe(1);
    expect(hb.skip).toBe(1);
    expect(hb.checkpoint).toBe("item-1");
  });
});

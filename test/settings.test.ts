...
import { loadSettings, loadSettingsFromRaw, type AppSettings } from "../scripts/catalog/settings.js"
...
describe("settings schema and safe defaults", () => {
  ...
  it("reads budgets and concurrency from config/settings.yml", () => {
    const s = loadSettingsFromRaw({
      ...
      concurrency: { github: 6, site: 3, llm: 2, model_probe: 2 },
    });
    ...
    expect(s.concurrency.llm).toBe(2);
    expect(s.concurrency.model_probe).toBe(2);
  });

  it("supports new env override names for local runs", () => {
    const s = loadSettings({
      CATALOG_GITHUB_CONCURRENCY: "7",
      CATALOG_SITE_CONCURRENCY: "5",
      CATALOG_LLM_CONCURRENCY: "2",
      CATALOG_MODEL_PROBE_CONCURRENCY: "3",
    ...
    expect(s.concurrency.llm).toBe(2);
    expect(s.concurrency.model_probe).toBe(3);
